...
namespace
{
    ...
    struct BenchOptions
    {
        ...
        std::vector<int> client_threads{1, 2, 4, 8, 16, 32};
    };
    ...
    BenchResult runOneConfig(const BenchOptions &options, int client_threads)
    {
        ...
        if (!pool.start())
        {
            std::cerr << "pool start failed, client_threads=" << client_threads << "\n";
            BenchResult failed;
            failed.pool_size = options.pool_size;
            failed.client_threads = client_threads;
        ...
        int calls_per_thread = options.total_requests / client_threads;
        int remainder = options.total_requests % client_threads;
    ...
    BenchOptions parseArgs(int argc, char *argv[])
    {
        ...
        if (argc >= 2) options.ip = argv[1];
        if (argc >= 3) options.port = static_cast<uint16_t>(std::stoi(argv[2]));
        if (argc >= 4) options.total_requests = std::stoi(argv[3]);
        if (argc >= 5) options.pool_size = std::stoi(argv[4]);
        if (argc >= 6) options.warmup_count = std::stoi(argv[5]);
        return options;
    }
    ...
    {
        std::cout
            << "| Mode | Pool Size | Client Threads | Provider Workers | Requests | "
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "client_threads_scaling_benchmark\n";
    ...
    for (int client_threads : options.client_threads)
    {
        BenchResult result = runOneConfig(options, client_threads);
